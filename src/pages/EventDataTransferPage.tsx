        ...
        <div style={{ ...cardStyle, display: "grid", gap: 14 }}>
          ...
          {options.attendance && <div style={{ marginLeft: 30, display: "grid", gap: 12, minWidth: 0 }}>
            <div role="radiogroup" aria-label="Как перенести ответы участников" style={{ display: "grid", gap: 10 }}>
              <RadioControl name="attendance-transfer-mode" checked={options.attendanceTransferMode === AttendanceTransferMode.ReplaceTarget} onChange={() => setOptions(current => ({ ...current, attendanceTransferMode: AttendanceTransferMode.ReplaceTarget }))} label="Заменить явку" disabled={submitting} />
              <RadioControl name="attendance-transfer-mode" checked={options.attendanceTransferMode === AttendanceTransferMode.MergePreferTarget} onChange={() => setOptions(current => ({ ...current, attendanceTransferMode: AttendanceTransferMode.MergePreferTarget }))} label="Объединить" disabled={submitting} />
              <RadioControl name="attendance-transfer-mode" checked={options.attendanceTransferMode === AttendanceTransferMode.ConfirmedOnly} onChange={() => setOptions(current => ({ ...current, attendanceTransferMode: AttendanceTransferMode.ConfirmedOnly }))} label="Перенести только «Смогу»" disabled={submitting} />
            </div>
